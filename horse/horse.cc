...
void config_init(const std::string cfg_path) {
    ...
    horse_cfg.client_async = ConfigManager::Instance().Get<bool>("client.async", false);
}
...
size_t read_file_to_buffer_queue(const std::string& file_path, BufferQueue<SegBuffer>& buffer_queue) {
    ...
    LOG_INFO() << __FUNCTION__ << "read finished. Total " << chunks_read << " chunks, " << total_read / 1024.0 << " KB";
    LOG_INFO() << "buffer queue currently contains " << buffer_queue.size() << " chunks" ;
    
    return total_read;
}
size_t async_read_and_send_file(const std::string& file_path, std::shared_ptr<StorageClient> Client){
    size_t total_read=0;
    BufferQueue<SegBuffer> buffer_queue(100);
    //TODO: create another thread to send the segment to server
    std::thread sender_thread([&buffer_queue, Client, file_path]() {
...
size_t read_and_send_file(const std::string& file_path, std::shared_ptr<StorageClient> Client) {
    std::ifstream file(file_path, std::ios::binary);
    if (!file.is_open()) {
        LOG_ERROR() << "cannot open file: " << file_path ;
        return 0;
    ...
    while (true) {
        ...
        std::vector<std::shared_ptr<SegBuffer>> buffers;
        do{
            uint64_t chunk_size = 0;
            if(cdc->calc_chunks(read_buffer.data(), read_buffer.remaining(), chunk_size) == -1){
                LOG_TRACE() << "hit boundary";
            ...
            std::shared_ptr<SegBuffer> pbuffer = std::make_shared<SegBuffer>(static_cast<size_t>(chunk_size));
            pbuffer->write(read_buffer.data(), chunk_size);
        ...
        for (int i=0;i<buffers.size();++i) {
            if (buffers[i]->size() > 0) {
                // TODO:Send buffer to server
                if(Client->FileWrite(file_path,buffers[i]->data(),buffers[i]->size())!=buffers[i]->size()){
                    LOG_ERROR() << "failed to write buffer["<<i<<"] to server, exit";
                    return 0;
                }
            }
        }
...
int main(int argc, char* argv[]) {
    ...
    std::shared_ptr<StorageClient> Client = std::make_shared<StorageClient>(
    ...
    size_t total_read=0;
    if(horse_cfg.client_async){
        total_read = async_read_and_send_file(file_path, Client);
    }else{
        total_read = read_and_send_file(file_path, Client);
    }
